...
numerator(a::QQMatrix) = integral_split(a, ZZ)[1]
...
function split_in_spinned_modules(M::AbstractSubModule) :: Vector{AbstractSubModule}
    # list of submodules generated by eᵢ for smallest i where eᵢ is not already
    # in the span of the previous submodules (via rref of bases)
    gs = codomain(M).action_of_gens
    @assert !isempty(gs)
    n = nrows(gs[1])
    R = base_ring(gs[1])
    @assert all(size(g) == (n,n) && base_ring(g) == R for g in gs)
    r = similar(gs[1], 0, n)
    k = 0
    cs = typeof(gs[1])[]
    while k < n
        e = _vector_not_in(r)
        c = Hecke.closure(e, gs)
        push!(cs, c)
        k, r = rref([r[1:k, :]; c])
    end
    return SubModule.((M,), cs)
end

"""
    _vector_not_in_span(A::MatrixElem)

First best unit vector not in the row span of the rref A.
"""
function _vector_not_in(A::MatrixElem)
    e = similar(A, 1, ncols(A))
    ...
    error("Matrix has full column rank")
end
